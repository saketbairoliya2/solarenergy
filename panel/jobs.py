...
def update_power_expected_table(response, each_unit):
	...
	try:
		...
		logger.info("No output found in response!!")
	return 1
...
def system_date_mapper(today_date, each_unit):
	'''
		This will receive date and System ID and will return simulated value of energy in watt for full day, passed date can be used for getting more refined output
	'''
...
def get_normal_distribution(capacity):
	...
	mu = (capacity_in_watt * decimal.Decimal(.45)) #Making an approximation for mean value to be 90% of system capacity
	sigma = mu/5 # As there are 13 numbers having data like bell curve, so making equal SD.
...
def performance_task(each_unit, date_now):
	...
	for each in time:
		try:
			...
			logger.info('Came accross no element in performance table.')
...
def expected_power(each_unit, date_now):
	...
	total_hours_to_ignore_for_eval = total_days_to_ignore_for_eval*24 - 1
